...
| Home service grid | Review Food, Groceries, Market Items, Parcel, SME Services, Pharmacy, Taxi and Bills tiles. | Active services navigate correctly; readiness/test services are clearly labelled. | Taxi public booking visible with flags off; Pharmacy live without approval. | Confirm no dense text menu returns. |
...
| Parcel Delivery | Create a parcel request as test. | Safe parcel flow captures pickup, delivery, recipient and package details. | Missing pickup/dropoff validation. | Do not use real recipient data. |
| SME Services | Create an SME Services request for an approved provider type. | Service-provider request captures category, service address, description, contact phone and optional schedule. | SME Services opens the parcel form; health professional booking is live without approval. | Health professional category must remain readiness-only. |
