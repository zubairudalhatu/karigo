...
const REFRESH_COOKIE = "karigo_admin_refresh";
...
function apiBaseUrl() {
  ...
  if (!configured?.trim() && productionPortal()) {
    throw new Error("Admin Portal BFF requires API_BASE_URL or NEXT_PUBLIC_API_BASE_URL in production.");
  }
...
function portalOrigins(request: NextRequest) {
  return [
    ...
    process.env.NEXT_PUBLIC_ADMIN_PORTAL_ORIGIN
...
export async function handleBffRequest(request: NextRequest, pathParts: string[]) {
  ...
  try {
    ...
    console.error(`Admin BFF backend request failed path=${path} reason=${error instanceof Error ? error.message : "unknown"}`);
  ...
  if (backendResponse.status === 401 && !publicAuthPath(path) && path !== "auth/logout") {
    ...
    if (refreshed?.accessToken) {
      try {
        ...
        console.error(`Admin BFF backend retry failed path=${path} reason=${error instanceof Error ? error.message : "unknown"}`);
        return jsonError("KariGO services are temporarily unavailable. Please try again shortly.", 503, "BFF_BACKEND_UNAVAILABLE");
      }
    }
  }
  ...
  if (accessFromPayload && role !== REQUIRED_ROLE) {
    const response = jsonError("This account is not authorised for the Admin Portal.", 403, "PORTAL_ROLE_REJECTED");
    clearSessionCookies(response);
    return response;
  }
