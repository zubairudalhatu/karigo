...
export default function SmeServicesPilotParticipantsPage() {
  ...
  async function createParticipant() {
    if (!form.displayName.trim()) {
      setError("SME participant could not be created. Please enter a display name.");
      return;
    }
    ...
    try {
      ...
      setForm(initialForm);
      setMessage("SME participant record has been created.");
      await load();
    } catch (e) {
      setError(friendlyError(e, "form") || "SME participant could not be created. Please check the required fields and try again.");
    } finally {
      setSaving(false);
    }
  }
  ...
  return <PortalShell>
    <h1>SME Services participants</h1>
    <p className="muted">Internal invitation list for SME Services customers, providers and observers. This page does not send SMS, email or WhatsApp invitations, activate automatic dispatch, collect payments or expose provider contacts publicly.</p>
    ...
    <section className="card section">
      ...
      <div className="form-grid">
        ...
        <label>Operations zone<input value={form.pilotZone} onChange={(e) => setForm({ ...form, pilotZone: e.target.value })} placeholder="Nasarawa GRA, Bompai, Tarauni" /></label>
    ...
    <div className="filters section">
      ...
      <input value={pilotZone} onChange={(e) => setPilotZone(e.target.value)} placeholder="Operations zone" />
    ...
    {loading ? <Loading /> : <section className="section">
      ...
      </Link>) : <Empty>No SME Services participants found.</Empty>}
    </section>}
  </PortalShell>;
}
