...
export default function AdminDashboard() {
  ...
  const metrics = data ? [["Total users", data.totalUsers], ["Customers", data.totalCustomers], ["Active vendors", data.activeVendors], ["Online captains", data.onlineRiders], ["Total orders", data.totalOrders], ["Orders today", data.ordersToday], ["Active orders", data.activeOrders], ["Completed", data.completedOrders], ["Cancelled", data.cancelledOrders], ["Failed", data.failedOrders], ["Pending tickets", data.pendingSupportTickets], ["Open refunds", data.openRefundRequests], ["GMV", money(data.grossMerchandiseValue)], ["Delivery fees", money(data.deliveryFeeTotal)], ["Pending vendor settlements", money(data.pendingVendorSettlements)], ["Pending captain earnings", money(data.pendingRiderEarnings)]] : [];
