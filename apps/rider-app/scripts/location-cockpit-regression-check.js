...
expect(dashboard.includes("recordLocalMapLocation(location)") && dashboard.includes("else if (!location.isApproximate) void uploadCaptainLocation") && dashboard.includes("void uploadCaptainLocation"), "Local watcher samples must update the marker before any precise conditional operational upload.");
expect(between(dashboard, "async function recenterLocalMap", "function promoteReadinessLocation").includes("acquireLocalMapLocation") && !between(dashboard, "async function recenterLocalMap", "function promoteReadinessLocation").includes("updateAvailability"), "Recenter must be local-only and must not mutate backend availability.");
...
expect(dashboard.includes("if (!projection.hasAnyActiveMode || !isForeground) return") && dashboard.includes("acquireLocalMapLocation()") && dashboard.includes(".then(promoteReadinessLocation)"), "Foreground open and resume must request a local map fix.");
