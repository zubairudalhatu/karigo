...
export function Screen({ children, title, refreshing, onRefresh }: { children: ReactNode; title?: string; refreshing?: boolean; onRefresh?: () => void }) { return <ScrollView contentContainerStyle={ui.screen} refreshControl={onRefresh ? <RefreshControl refreshing={!!refreshing} onRefresh={onRefresh} tintColor={brand.colors.primary} /> : undefined}>{title ? <Text style={ui.title}>{title}</Text> : null}{children}</ScrollView>; }
...
export function PasswordField(props: React.ComponentProps<typeof TextInput> & { visible: boolean; onToggleVisible: () => void }) { const { visible, onToggleVisible, ...inputProps } = props; return <View style={ui.passwordWrap}><TextInput accessibilityLabel={inputProps.placeholder} placeholderTextColor={brand.colors.muted} {...inputProps} secureTextEntry={!visible} style={[ui.input, ui.passwordInput, inputProps.style]} /><Pressable accessibilityRole="button" accessibilityLabel={visible ? "Hide password" : "Show password"} onPress={onToggleVisible} style={ui.passwordToggle}><Text style={ui.passwordToggleText}>{visible ? "Hide" : "Show"}</Text></Pressable></View>; }
...
export function StatusBadge({ status }: { status: string }) { const normalized = status.replaceAll(" ", "_").toUpperCase(); const label=status.replaceAll("_"," ").toLowerCase().replace(/\b\w/g,l=>l.toUpperCase());const tone=["COMPLETED","PAID","DELIVERED","ONLINE","AVAILABLE","ACTIVE"].includes(normalized)?ui.badgeSuccess:["FAILED","CANCELLED","VENDOR_REJECTED","OFFLINE","UNAVAILABLE","SUSPENDED"].includes(normalized)?ui.badgeDanger:["PREPARING","READY_FOR_PICKUP","ON_THE_WAY","RIDER_ASSIGNED","BUSY","ON_DELIVERY","RIDER_ARRIVING_PICKUP"].includes(normalized)?ui.badgeWarning:ui.badgeInfo;return <Text style={[ui.badge,tone]}>{label}</Text>; }
