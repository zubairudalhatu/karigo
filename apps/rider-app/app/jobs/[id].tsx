...
export default function JobDetails() {
  ...
  const isCashPod = job?.paymentMethod === "CASH_ON_DELIVERY" || job?.paymentStatus === "CASH_PENDING";
  ...
  return <Protected><Screen title={job?.orderNumber ?? "Job details"} subtitle="Follow the dispatch-approved delivery sequence." refreshing={loading} onRefresh={load}><Message error>{error}</Message><Message>{success}</Message>{job ? <>
    <Card tone="soft"><Text style={ui.sectionTitle}>Current status</Text><StatusBadge status={job.orderStatus} /><Text>{job.vendor?.businessName ?? job.serviceCategory}</Text><Text style={ui.muted}>{job.itemDescription || "Delivery order"} - {money(job.deliveryFee)} delivery fee</Text>{isCashPod ? <><Text style={ui.muted}>Payment method: Cash / Pay on Delivery</Text><Text style={ui.muted}>Amount to collect: {money(job.totalAmount ?? 0)}</Text></> : <Text style={ui.muted}>Payment method: {job.paymentMethod === "WALLET" ? "KariGO Wallet" : "Online payment"}</Text>}{job.customerNote ? <Text style={ui.muted}>Customer note: {job.customerNote}</Text> : null}</Card>
    ...
    {job.orderStatus === "DELIVERED" ? <Card><Text style={ui.title}>Complete with customer OTP</Text><Text style={ui.muted}>Ask the customer to reveal the six-digit delivery code. Do not request or store it anywhere else.</Text>{isCashPod ? <><Text style={ui.muted}>Cash/POD collection</Text><Text style={ui.muted}>Confirm you collected exactly {money(job.totalAmount ?? 0)} before completing this job.</Text><Button tone={cashCollected ? "primary" : "muted"} title={cashCollected ? "Cash collected confirmed" : "Confirm cash collected"} onPress={() => setCashCollected((current) => !current)} /></> : null}<Field keyboardType="number-pad" maxLength={6} value={otp} onChangeText={(value) => setOtp(value.replace(/\D/g, "").slice(0, 6))} placeholder="6-digit delivery OTP" /><Button disabled={otp.length !== 6 || (isCashPod && !cashCollected)} title="Complete delivery" onPress={async () => { if (await action(() => jobsApi.complete(job.id, otp, isCashPod ? cashCollected : undefined), "Delivery completed successfully.")) router.replace("/earnings"); }} /></Card> : null}
