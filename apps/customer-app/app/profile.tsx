...
export default function Profile() {
  ...
  return <Protected><Screen title="Your profile"><Message error>{error}</Message>{profile ? <>
    ...
    <Card><Text style={ui.title}>Your KariGO activity</Text><Text>{retention?.totalOrders ?? 0} orders · {retention?.completedOrders ?? 0} completed</Text><Text>{retention?.promoUsageCount ?? 0} promos used</Text></Card>
