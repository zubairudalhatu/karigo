...
export default function OrderTracking() {
  ...
  return <Protected><Screen title={order?.orderNumber ?? "Order tracking"}><Message>{message}</Message><Message error>{error}</Message>
    {order ? <><Card><StatusBadge status={order.orderStatus} /><Text>Payment: {order.paymentStatus}</Text><Text>Total: {money(order.totalAmount)}</Text>{order.vendor ? <Text>{order.vendor.businessName}</Text> : null}</Card>
      {order.paymentStatus === "PENDING" ? <Button title={busy ? "Verifying..." : "Pay with mock provider"} onPress={pay} disabled={busy} /> : null}
      {order.items?.map((item) => <Card key={item.id}><Text>{item.productName} × {item.quantity}</Text><Text>{money(item.totalPrice)}</Text></Card>)}
