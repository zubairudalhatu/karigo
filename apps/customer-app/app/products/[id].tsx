...
export default function ProductDetails() {
  ...
  return <Screen title={product?.name ?? "Product"}><Message error>{error}</Message>{product ? <Card>
    ...
    <Button title={!product.isAvailable ? "Unavailable" : !user ? "Sign in to add" : cart.addingProductIds.includes(product.id) ? "Added" : `Add ${quantity} to cart`} disabled={!product.isAvailable || !vendor || (!!user && cart.addingProductIds.includes(product.id))} onPress={() => {
      if (!user) {
