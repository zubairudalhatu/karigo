...
export default function Notifications() {
  ...
  return <Protected><Screen title="Activity feed" actions={<Button title="Read all" tone="muted" onPress={async () => { await notificationsApi.markAllRead(); await load(); }} />}><Message error>{error}</Message>
    {items.length === 0 ? <Empty message="Your KariGO updates will appear here." /> : items.map((item) => <Pressable key={item.id} onPress={async () => { if (!item.isRead) { await notificationsApi.markRead(item.id); await load(); } if (item.type === "SYSTEM_ALERT" && item.entityType === "TaxiTrip" && item.entityId) router.push(`/taxi/chat/${item.entityId}` as never); }}><Card><Text style={ui.title}>{item.isRead ? "" : "New · "}{item.title}</Text><Text>{item.message}</Text><Text style={ui.muted}>{new Date(item.createdAt).toLocaleString()}</Text></Card></Pressable>)}
  </Screen></Protected>;
}
