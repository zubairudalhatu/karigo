...
assert(checkout.includes("Pay with Squad"), "Checkout must show live Squad payment button copy when Squad live mode is active.");
assert(checkout.includes("paymentProvider: selectedPaymentProvider"), "Checkout must send the selected payment provider to the backend.");
...
assert(paymentStatus.includes("Wallet top-up is controlled by backend config; wallet order payment, automatic refunds and payout automation remain disabled"), "Payment copy must keep wallet top-up controlled and wallet order payment disabled.");
