...
assert(home.includes("KariGO Rides"), "Home must include the KariGO Rides tile.");
assert(home.includes("taxiApi.trips"), "Home must reconcile active KariGO Rides for the compact status entry.");
assert(home.includes("isActiveTaxiTripStatus"), "Home active Ride entry must use shared active lifecycle classification.");
assert(home.includes("Active KariGO Ride"), "Home must show a compact active Ride entry when a trip is active.");
...
assert(taxiRequest.includes("Location.reverseGeocodeAsync"), "Ride request flow may reverse-geocode center-pin/current locations.");
assert(taxiRequest.includes("taxiApi.placesAutocomplete"), "Ride request flow must search through the backend Places proxy.");
...
assert(taxiRequest.includes("styles.routeInput") && taxiRequest.includes("TextInput"), "Route composer rows must render active search inputs inline.");
assert(!taxiRequest.includes("Search pickup\" : activeField === \"stop\" ? \"Search stop\" : \"Search destination\""), "Ride planning must not render the duplicate standalone destination search field.");
assert(!taxiRequest.includes("styles.addressInput"), "Ride planning must remove the old standalone address input style.");
assert(!taxiRequest.includes("Active ride requests"), "Ride tracking must not show active ride lists.");
assert(!taxiRequest.includes("Ride history"), "Ride tracking must not show ride history.");
assert(!taxiRequest.includes("RideActiveBanner"), "Ride Home must not show the old in-flow active ride banner while entry guard owns active rides.");
assert(!taxiRequest.includes("RideActiveList"), "Ride tracking must not render the legacy active ride list.");
assert(!taxiRequest.includes("RideHistoryList"), "Ride tracking must not render the legacy ride history list.");
...
assert(taxiRequest.includes("CaptainSearchProgress"), "Requested ride tracking must use an indeterminate Captain-search progress treatment.");
assert(taxiRequest.includes("Searching for an available KariGO Ride Captain."), "Captain-search progress must have an accessibility label.");
assert(taxiRequest.includes("/orders?tab=rides"), "Ride tracking must link to Orders -> Rides for all ride management.");
assert(taxiRequest.includes('router.replace("/tabs/home")'), "Ride tracking top actions must return to KariGO Home instead of Ride Home.");
assert(!taxiRequest.includes("Pay Now"), "Ride Test Mode must not show a payment action.");

const ordersIndex = read("app", "orders", "index.tsx");
assert(ordersIndex.includes("type OrdersTab = \"orders\" | \"rides\""), "Orders screen must support an Orders/Rides tab model.");
assert(ordersIndex.includes("taxiApi.trips"), "Orders -> Rides must load customer ride trips from the existing backend API.");
assert(ordersIndex.includes("Active rides"), "Orders -> Rides must include active ride requests.");
assert(ordersIndex.includes("Ride history"), "Orders -> Rides must include terminal ride history.");
assert(ordersIndex.includes("taxiApi.cancelTrip"), "Orders -> Rides must cancel the selected ride by trip ID.");
assert(ordersIndex.includes("/taxi/request?tripId=${trip.id}"), "Active rides in Orders must open the exact tracking screen.");
...
assert(utilityFlow.includes("Insufficient KariGO Wallet balance. Add at least"), "Utility flow must show exact insufficient wallet shortage state.");
...
assert(utilityFlow.includes("Mobile network") && utilityFlow.includes("Distribution company") && utilityFlow.includes("TV provider"), "Utility flow must render service-specific provider labels.");
assert(utilityFlow.includes("Data plan") && utilityFlow.includes("Bouquet or package"), "Utility flow must render package selectors for data and cable TV.");
assert(utilityFlow.includes("Recipient phone number") && utilityFlow.includes("Meter number") && utilityFlow.includes("Smartcard / IUC / decoder number"), "Utility flow must label recipient fields accurately.");
assert(utilityFlow.includes("Utility provider connection is temporarily unavailable. Please try again later."), "Utility flow must show provider connection unavailable state.");
assert(utilityFlow.includes("Retry provider connection") && utilityFlow.includes("Retry catalogue"), "Utility flow must expose retry actions for provider/catalogue failures.");
assert(utilityFlow.includes("reviewDisabledReason"), "Utility review eligibility must be explicit and testable.");
assert(utilityFlow.includes("Select a ${config.providerLabel.toLowerCase()}."), "Utility flow must explain missing provider selection.");
assert(utilityFlow.includes("Select a ${config.productLabel?.toLowerCase()"), "Utility flow must explain missing package selection.");
assert(utilityFlow.includes("Enter a valid Nigerian phone number."), "Utility flow must validate Nigerian phone numbers.");
assert(utilityFlow.includes("Enter a valid meter number.") && utilityFlow.includes("Enter a valid smartcard or IUC number."), "Utility flow must validate meter and smartcard fields.");
assert(utilityFlow.includes("All required fields are ready. Review before submitting."), "Utility flow must show a ready review state.");
assert(utilityFlow.includes("Opening Review does not submit the request."), "Utility review screen must not submit prematurely.");
assert(utilityFlow.includes("Wallet before:") && utilityFlow.includes("Wallet after:"), "Utility review must show wallet balance before and after payment.");
assert(utilityFlow.includes("Edit details"), "Utility review must expose an edit action before confirmation.");
assert(utilityFlow.includes("cancelInlineTransaction"), "Utility flow must support inline receipt cancellation for eligible requests.");
assert(utilityFlow.includes("This utility request failed and your wallet has been reversed."), "Utility flow must show reversal copy for failed provider fulfilment.");
assert(utilityFlow.includes("This utility request failed. If your wallet was debited, KariGO will reverse it automatically."), "Utility flow must not show processing copy for failed utility requests.");
assert(utilityFlow.includes("This utility request was cancelled before fulfilment."), "Utility flow must show cancellation copy when a request is cancelled.");
assert(utilityFlow.includes("This utility request was cancelled and your wallet has been reversed."), "Utility flow must distinguish cancelled wallet reversals.");
...
assert(utilityReceipt.includes("Keep request") && utilityReceipt.includes("Cancel request"), "Utility cancellation confirmation must include safe action labels.");
assert(utilityReceipt.includes("utilitiesApi.cancel"), "Utility receipt must use the existing cancellation endpoint.");
assert(utilityReceipt.includes("setTransaction(cancelled)"), "Utility receipt must refresh local receipt state after cancellation.");
