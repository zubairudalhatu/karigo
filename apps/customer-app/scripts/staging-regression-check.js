...
assert(walletScreen.includes("squadCustomerCheckoutEnabled"), "Customer wallet top-up must also require the emergency Squad customer checkout flag.");
...
assert(walletScreen.includes("Wallet top-up is temporarily unavailable."), "Customer wallet screen must show the temporary top-up unavailable copy.");
...
assert(!checkout.includes("openExternalPaymentUrl"), "POD-only checkout must not open external payment URLs.");
assert(!checkout.includes("paymentsApi.initiate"), "POD-only checkout must not initiate electronic payment.");
assert(!checkout.includes("Pay with Squad"), "POD-only checkout must hide Squad customer checkout.");
assert(!checkout.includes("Pay from Wallet"), "POD-only checkout must hide wallet order payment.");
assert(!checkout.includes("paymentProvider: selectedPaymentProvider"), "POD-only checkout must not send a customer electronic payment provider.");
assert(paymentStatus.includes("Mock Payment is for staging fallback"), "Payment copy must state mock is staging fallback only.");
assert(paymentStatus.includes("Squad by GTBank"), "Payment copy must include Squad launch wording.");
assert(paymentStatus.includes("isSquadLivePaymentConfig"), "Customer checkout must recognize backend-confirmed Squad live mode.");
assert(paymentStatus.includes("squadCustomerCheckoutEnabled"), "Customer payment config must include the emergency Squad customer checkout flag.");
...
assert(paymentStatus.includes("customerSelectableProviders: []"), "Customer production fallback must not expose electronic providers during POD-only fallback.");
...
assert(paymentStatus.includes("Customer checkout is currently Pay on Delivery only while electronic checkout is under live review."), "Payment copy must state the emergency POD-only checkout posture.");
assert(!checkout.includes("Verify payment status"), "POD-only checkout must not expose electronic payment verification.");
assert(!checkout.includes("paymentInitializationFailureMessage"), "POD-only checkout must not show electronic provider initialization copy.");
...
assert(checkout.includes('paymentMethod: "CASH_ON_DELIVERY"'), "Pay on Delivery order creation must always send CASH_ON_DELIVERY.");
...
assert(!orderDetail.includes("Pay with Squad"), "Order detail must not render Squad payment button copy during POD-only fallback.");
