...
const LEGACY_TOKEN_KEY = "karigo_customer_access_token";
const LEGACY_REFRESH_TOKEN_KEY = "karigo_customer_refresh_token";
const SESSION_KEY = "karigo_customer_session_v2";
const API_BASE_URL = validateMobileApiBaseUrl(
...
async function performRefreshAuth(operationGeneration?: number): Promise<RefreshAuthResult> {
  if (!authSessionStore.isCurrent(operationGeneration)) {
    logMobileAuthDiagnostic("customer", "refresh_stale_before_start", { generation: operationGeneration });
    return "stale";
  }

  const session = await authSessionStore.readSession();
  if (!session?.refreshToken) {
    logMobileAuthDiagnostic("customer", "refresh_no_saved_session", { generation: operationGeneration });
    return "rejected";
  }
  ...
  try {
    ...
    if (!response.ok || !payload?.success) {
      ...
      if (refreshFailureIsTemporary(response.status, errorCode)) {
        logMobileAuthDiagnostic("customer", "refresh_temporary_failure", {
      ...
      if (refreshFailureIsDefinitive(response.status, errorCode)) {
        ...
        logMobileAuthDiagnostic("customer", "refresh_rejected", {
          status: response.status,
          errorCode,
          generation: operationGeneration
        });
      ...
      logMobileAuthDiagnostic("customer", "refresh_unclassified_failure", {
        status: response.status,
        errorCode,
        generation: operationGeneration
      });
    ...
    if (!nextAccessToken || !nextRefreshToken) {
      logMobileAuthDiagnostic("customer", "refresh_missing_token_pair", { generation: operationGeneration });
      return "temporarily_unavailable";
    }

    await authSessionStore.persistTokenPair(nextAccessToken, nextRefreshToken, operationGeneration);
    logMobileAuthDiagnostic("customer", "refresh_succeeded", { generation: operationGeneration });
    return "refreshed";
  } catch (error) {
    if (error instanceof StaleAuthOperationError) {
      logMobileAuthDiagnostic("customer", "refresh_stale_after_start", { generation: operationGeneration });
      return "stale";
    }
    if (error instanceof SessionCorruptionError || error instanceof SessionPersistenceError) {
      throw error;
    }
    logMobileAuthDiagnostic("customer", "refresh_network_or_timeout", { generation: operationGeneration });
    return "temporarily_unavailable";
  }
}
...
async function refreshAuth(meta: { generation?: number }): Promise<RefreshAuthResult> {
  ...
  if (
    refreshFlight &&
    refreshFlight.generation === operationGeneration &&
    authSessionStore.isCurrent(operationGeneration)
  ) {
    logMobileAuthDiagnostic("customer", "refresh_joined_existing", { generation: operationGeneration });
...
export const api = createApiClient({
  ...
  onUnauthorized: async (status, meta) => {
    ...
    if (authSessionStore.isCurrent(meta?.generation)) {
      logMobileAuthDiagnostic("customer", "unauthorized_session_cleared", {
        status,
      ...
      logMobileAuthDiagnostic("customer", "unauthorized_stale_ignored", {
        status,
        generation: meta?.generation
      });
    }
