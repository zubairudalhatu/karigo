...
export default function Products() {
  ...
  function validateForm() {
    if (form.name.trim().length < 2) return "Enter a product name with at least 2 characters.";
    if (form.description.trim().length < 8) return "Enter a product description with at least 8 characters.";
    if (!Number.isFinite(Number(form.price)) || Number(form.price) < 1) return "Product price must be at least NGN 1.";
    try {
      const imageUrl = new URL(form.imageUrl);
      const imagePath = imageUrl.pathname.toLowerCase();
      const isImage = [".jpg", ".jpeg", ".png", ".webp"].some((extension) => imagePath.endsWith(extension)) || imagePath.includes("/uploads/vendors/");
      if (imageUrl.protocol !== "https:" || !isImage) return "Use a valid HTTPS JPG, PNG or WebP product image.";
    } catch {
      return "Use a valid HTTPS JPG, PNG or WebP product image.";
    }
    return "";
  }
  ...
  return <DashboardShell>
    ...
    <p className="notice">Only active, approved product-selling partners can create, update, publish or archive products. Pending, suspended or service-only partners should contact KariGO Operations or use the Services workspace.</p>
    ...
    <section className="product-layout">
      <form className="card product-form" onSubmit={submit}>
        ...
        <button disabled={saving || uploadingImage}>{saving ? "Saving..." : editing ? "Save changes" : "Create product"}</button>{editing ? <button className="secondary" type="button" onClick={resetForm}>Cancel edit</button> : null}
      </form>
