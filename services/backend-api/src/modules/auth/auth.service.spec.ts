...
describe("AuthService", () => {
  ...
  it("guides existing Customer accounts to sign in for Captain onboarding instead of duplicating the account", async () => {
    usersService.findByPhoneForAuth.mockResolvedValue({
      id: "customer-user-1",
    ...
    const result = await service.createApplicantAccount(UserRole.RIDER, {
      fullName: "Existing Customer",
      phoneNumber: "08012345678",
      email: "customer@example.test"
    });
    ...
    expect(result).toMatchObject({
      ...
      message: "This phone number already has a KariGO account. Sign in with your existing KariGO password to continue your Captain application.",
      account: {
        id: "customer-user-1",
        role: UserRole.CUSTOMER,
        phoneVerified: true
