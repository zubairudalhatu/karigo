...
export class ApplicationNotificationsService {
  ...
  async vendorApplicationSubmitted(input: ApplicationNotificationInput): Promise<void> {
    const message = `KariGO has received your vendor application ${input.reference} for Kano and Abuja launch onboarding review. Approval is not automatic. We will contact you with next steps.`;
  ...
  async orderCreated(input: ApplicationNotificationInput): Promise<void> {
    const message = `KariGO order ${input.reference} has been created and is awaiting payment confirmation in the app. KariGO will only mark the order paid after backend payment verification or approved operations reconciliation.`;
